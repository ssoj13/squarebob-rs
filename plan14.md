...
Initial documentation checkpoint: 2026-09-23, based on committed `main` at `6562a5280c42195345dee2fef36c59254ee7d894`. The observations below describe that checkpoint. Publication update: `fscan-rs` was published privately at `aa0185f8a45dcf3ad949d97b88b5950009444cb2`, Squarebob pinned that Git revision in published commit `e662a71b5b127b702e71e803a7a4053d3285fdcb`, and its scanner unit tests and `cargo check` passed on Windows. Large-scan runtime parity and packaging remain unverified.
...
- [x] Recheck the standard and NTFS scanner adapters, update active dataflows, and confirm compilation at the initial local-path checkpoint.
...
- `README.md`: current dependency versions from `Cargo.toml:52-74,105-116`; the workspace member list from `Cargo.toml:15-37`; CI cancellation behavior from `.github/workflows/ci.yml:31-35`; the private `fscan-rs` Git revision pinned in `Cargo.toml:179` and `Cargo.lock:3337-3339`; and links to architecture, bug reports, and historical design records.
...
- [x] Restore the typed NTFS failure contract across the `fscan-rs` boundary. Published `fscan-rs` commit `c33d447` preserves `ScanFailure` through `scan_ntfs_tree_with_options` (`../fscan-rs/src/ntfs.rs:394-433`); the current adapter cancels, falls back only for `BackendUnavailable`, and fails terminally for `Failed` (`src/scanner_ntfs.rs:139-154`). This is a source and compilation result; runtime parity remains open in [plan15.md](plan15.md).
- Restore CI access to the private `oiio-rs` Git dependency. The earlier jobs failed on SSH authentication before Clippy; no CI run was started for the later NTFS repair. Neither result establishes Clippy or packaging status.
- Run the Linux and Windows portable packaging branches at a justified CI gate; the source-level name repair alone does not establish packaging success.

## Documentation verification

Every changed Markdown file was reread after editing. All 16 unique local link targets referenced by those files exist; Markdown fences are balanced. `git diff --check` exited 0. No Markdown linter or Prettier executable is installed in PATH, so no linter command was run. At the initial local-path checkpoint, the root agent ran `cargo check --workspace --locked -q`; it exited 0 with empty stderr. After the first Git pin was published, scanner unit tests and `cargo check` passed on Windows. The later typed NTFS repair and cleanup of incidental lockfile changes are tracked in [plan15.md](plan15.md). No full application run, visual comparison, large-scan benchmark, or packaging workflow was performed for this documentation pass.
