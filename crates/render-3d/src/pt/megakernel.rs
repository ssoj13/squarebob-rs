...
use pt_mats::{
    classify_path_filtered, MaterialClass, MaterializeMode, MaterializeSettings,
};
use render_core::gpu;
...
pub(crate) fn render_path_traced_no_readback(
        ...
        use pt_core::gpu_data::{build_instance_gpu_data, build_gpu_data_from_nodes, GpuInstanceSceneData};


        let surface_format = wgpu::TextureFormat::Rgba8Unorm;
        if renderer.pt.path_tracer.is_none() {
            let mut pt = PathTraceCompute::new(
                &renderer.ctx.device, &renderer.ctx.queue, width, height, surface_format,
            );
            if opts.env_map_enabled {
                pt.set_environment_texture(
                    &renderer.ctx.device, &renderer.ctx.queue,
                    &renderer.env.texture, opts.env_map_intensity, true,
                );
        ...
        let snap_interval = if snap_enabled { 1.0 / opts.pt_target_fps.max(1.0) } else { 0.0 };
        let elapsed = if snap_enabled { renderer.pt.pt_camera_snap_time.elapsed().as_secs_f32() } else { 0.0 };
        let allow_update = !snap_enabled || elapsed >= snap_interval || !renderer.pt.pt_snap_valid || pt.frame_count == 0;
        ...
        if allow_update && (renderer.pt.pt_scene_dirty || animated) && !instances.is_empty() {
            ...
            let mut light_variant_ids: std::collections::HashMap<u64, u32> = std::collections::HashMap::new();
            ...
            let default_id = renderer.material_library.material_id(MaterialClass::Default);
                ...
                .map(|inst| {
                    ...
                    let material_id = if opts.materialize_mode != MaterializeMode::None {
                        if let Some(path) = renderer.picking.path_for_id(inst.object_id) {
                            let is_dir = renderer.picking.is_dir_for_id(inst.object_id).unwrap_or(false);
                            if is_dir && !opts.mat_include_dirs {
                                ...
                                if class.is_light() && (light_intensity != 1.0 || light_color_rand > 0.0) {
                                    let bucket = if light_variants > 1 { hash % light_variants } else { 0 };
                                    ...
                                    if let Some(&id) = light_variant_ids.get(&key) {
                                        ...
                                        if light_color_rand > 0.0 {
                                            let r = 1.0 + (hash_f32(hash, 0xA1u32) - 0.5) * 2.0 * (0.3 * light_color_rand);
                                            let g = 1.0 + (hash_f32(hash, 0xB2u32) - 0.5) * 2.0 * (0.3 * light_color_rand);
                                            let b = 1.0 + (hash_f32(hash, 0xC3u32) - 0.5) * 2.0 * (0.3 * light_color_rand);
                                            m.emission_color_weight[0] = (m.emission_color_weight[0] * r).max(0.0);
                                            m.emission_color_weight[1] = (m.emission_color_weight[1] * g).max(0.0);
                                            m.emission_color_weight[2] = (m.emission_color_weight[2] * b).max(0.0);
                                        }
                                        materials.push(m);
            ...
            if opts.pt_gpu_bvh && opts.pt_bvh_refit {
                let data = GpuInstanceSceneData { nodes: Vec::new(), instances: Vec::new(), materials: materials.clone() };
                pt.upload_scene_smart(&renderer.ctx.device, &renderer.ctx.queue, &pt_instances, &data, animated);
                reset_by_scene = true;
                pt.reset_accumulation();
            } else {
                let gpu_data = if opts.pt_gpu_bvh {
                    let (nodes, sorted_indices) = pt.build_bvh(
                        &renderer.ctx.device, &renderer.ctx.queue, &pt_instances
                    );
        ...
        let cam_moved = (pt.last_camera_pos != Some(cam_pos))
        ...
        let slice_params = (opts.slice_enabled, compute_slice_normal(opts), compute_slice_position(opts), opts.slice_invert);
        ...
        let anim_time_used = if snap_enabled {
            if allow_update { opts.animation_time } else { renderer.pt.pt_snap_anim_time }
        } else {
            opts.animation_time
        };
        let env_time = if opts.env_animate { anim_time_used * opts.env_speed } else { 0.0 };
        ...
        pt.set_restir_options(opts.pt_restir_temporal, opts.pt_restir_spatial, opts.pt_restir_m_max);
        pt.set_adaptive_enabled(&renderer.ctx.device, &renderer.ctx.queue, opts.pt_adaptive_sampling);
        ...
        // Dispatch + blit (no readback!)
        let mut encoder = renderer.ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("PT Encoder (zero-copy)"),
        });

        let samples = if opts.pt_auto_spp { renderer.pt.pt_samples_per_update } else { opts.pt_samples_per_update.max(1) };
        log::debug!("PT: samples={}, auto_spp={}, wavefront={}, frame_count={}", samples, opts.pt_auto_spp, opts.pt_wavefront, pt.frame_count);
...
pub(crate) fn render_path_traced(
        ...
        if allow_update && (renderer.pt.pt_scene_dirty || animated) && !instances.is_empty() {
            ...
            debug!("PT BVH start: gpu={}, instances={}", opts.pt_gpu_bvh, pt_instances.len());
            let gpu_data = if opts.pt_gpu_bvh && opts.pt_bvh_refit {
                ...
                Some(build_gpu_data_from_nodes(nodes, &sorted_indices, &pt_instances, &materials))
            } else {
                let bvh = build_instance_bvh(&pt_instances);
                debug!(
            ...
            debug!("  bvh_build: {:.2}ms ({})", bvh_ms, if opts.pt_gpu_bvh { "GPU" } else { "CPU" });
            ...
            debug!("collect_cubes: {:.2}ms ({} cubes)", scene_ms, pt_instances.len());
        }
        ...
        let output_buffer = gpu::readback_texture(
            &renderer.ctx, &mut encoder, &targets.render_texture, width, height,
        );
