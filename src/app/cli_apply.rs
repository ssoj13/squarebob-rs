...
mod tests {
    ...
    fn every_some_flag_lands_in_expected_field() {
        ...
        assert_eq!(opts.path_tracing, true);
        assert_eq!(opts.pt_wavefront, true);
        assert_eq!(opts.pt_wavefront_tile_size, 64);
        assert_eq!(opts.pt_max_bounces, 13);
        assert_eq!(opts.pt_max_samples, 123);
        assert_eq!(opts.pt_gpu_bvh, true);
        assert_eq!(opts.pt_path_guiding, true);
        assert_eq!(opts.pt_restir_di, true);
        assert_eq!(opts.pt_restir_gi, true);
        assert_eq!(opts.pt_adaptive_sampling, true);
        assert_eq!(opts.env_map_enabled, true);
        assert_eq!(opts.show_wireframe, true);
        assert_eq!(opts.animate, true);
        assert!(matches!(opts.height_mode, CubeHeightMode::Depth));
        assert_eq!(opts.height_power_enabled, true);
        ...
        assert_eq!(opts.flat_shading, true);
        assert_eq!(opts.double_sided, true);
        assert!(matches!(opts.materialize_mode, MaterializeMode::ByExtension));
        assert_eq!(opts.mat_allow_lights, true);
        assert_eq!(opts.mat_light_prob, 0.77);
        assert_eq!(opts.mat_allow_glass, true);
        ...
        assert_eq!(opts.env_map_visible, true);
        assert_eq!(opts.env_map_path, Some(PathBuf::from("/tmp/env.hdr")));
        assert_eq!(opts.env_animate, true);
        ...
        assert_eq!(opts.pt_dof_enabled, true);
        assert_eq!(opts.pt_aperture, 0.05);
        assert_eq!(opts.pt_focus_distance, 2.5);
        assert_eq!(opts.pt_env_importance_sampling, true);
        assert_eq!(opts.pt_target_fps, 45.0);
        assert_eq!(opts.pt_auto_spp, true);
        assert_eq!(opts.pt_camera_snap, true);
        assert!(matches!(opts.pt_spectral_mode, SpectralMode::Hero));
        assert_eq!(opts.pt_spectral_samples, 7);
        assert_eq!(opts.pt_spectral_dispersion, true);
        assert_eq!(opts.pt_bvh_refit, true);
        assert_eq!(opts.pt_russian_roulette, true);
        assert_eq!(opts.pt_restir_temporal, true);
        assert_eq!(opts.pt_restir_spatial, true);
        assert_eq!(opts.pt_restir_m_max, 20);
        assert_eq!(opts.pt_svo_resolution, 128);
        assert_eq!(opts.slice_enabled, true);
        assert_eq!(opts.slice_axis, 2);
        assert_eq!(opts.slice_position, 0.5);
        assert_eq!(opts.slice_position_vector, 0.75);
        assert_eq!(opts.slice_invert, true);
        assert_eq!(opts.slice_use_vector, true);
        assert_eq!(opts.slice_normal, [0.0, 1.0, 0.0]);
        assert_eq!(opts.lod_enabled, true);
        assert_eq!(opts.lod_min_screen_size, 4.0);
        assert_eq!(opts.inertia_enabled, true);
